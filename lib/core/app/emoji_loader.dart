...
///
/// 数据通过 [DatabaseHelper] 持久化，按站点 host 隔离。
class EmojiService {
  ...
  List<Map<String, dynamic>> get frequentlyUsed {
    ...
    for (final e in entries) {
      ...
      if (!idMap.containsKey(smilieId)) continue;
  ...
  Future<void> _loadUsage() async {
    try {
      final json = await _db.getMeta('emoji_usage_$_host');
  ...
  /// 从 DatabaseHelper 恢复缓存，返回是否成功。
  /// 若 CDN 配置已变更则丢弃缓存，触发重新请求。
  Future<bool> _loadFromCache() async {
    try {
      final json = await _db.getMeta('smilie_data_$_host');
